...
A persistência ficou desde o início em **SQLite**, com o schema versionado em [`backend/data/schema_v3.sql`](backend/data/schema_v3.sql) como fonte única da verdade.
...
├───────────────────────────────────────────────────────────────┤
│  SQLite (backend/data/sigeh_v3.db)                               │
└───────────────────────────────────────────────────────────────┘
```
...
└── backend/
    ├── data/                         # dados
    │   ├── schema_v3.sql             # schema único (fonte da verdade)
    ...
    └── web/                          # backend web
        ...
        │   └── senha_util.{h,c}       # hashing PBKDF2-HMAC-SHA256 + salt (OpenSSL)
        ├── api/
