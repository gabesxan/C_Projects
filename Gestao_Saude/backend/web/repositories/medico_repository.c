...
int medico_repo_ids_por_especialidade(const char *especialidade, int *ids, int max)
{
    ...
    const char *sql =
        "SELECT id FROM medicos "
        "WHERE ativo = 1 AND especialidade = ? "
        "ORDER BY id;";
    int total = 0;

    if (ids == NULL || max <= 0 || especialidade == NULL || especialidade[0] == '\0')
    {
        return 0;
    }
...
int medico_repo_distribuicao_por_especialidade_json(char *buffer, int tamanho)
{
    ...
    const char *sql =
        ...
        "WHERE ativo = 1 GROUP BY especialidade ORDER BY especialidade;";
    int usado = 0;
    int primeiro = 1;
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        if (escaparJson(especialidadeJson, sizeof(especialidadeJson),
                        especialidade) == 0)
        {
            sqlite3_finalize(stmt);
            db_fechar(db);
...
int medico_repo_ativo(int id)
{
    sqlite3 *db = NULL;
    sqlite3_stmt *stmt = NULL;
    const char *sql = "SELECT 1 FROM medicos WHERE id = ? AND ativo = 1;";
    int ativo = 0;
