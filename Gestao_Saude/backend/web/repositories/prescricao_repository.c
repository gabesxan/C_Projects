                          ...
                          const char *frequencia, const char *observacoes)
{
    ...
    const char *sql =
        ...
        "(paciente_id, medico_id, medicamento, dosagem, frequencia, "
        "observacoes, ativo) VALUES (?, ?, ?, ?, ?, ?, 1);";
    ...
    sqlite3_bind_text(stmt, 6, observacoes, -1, SQLITE_STATIC);
                                ...
                                char *buffer, int tamanho)
{
    ...
    if (filtroCol != NULL)
    {
        snprintf(sql, sizeof(sql),
            ...
            "frequencia, observacoes FROM prescricoes "
            "WHERE ativo = 1 AND %s = ? ORDER BY id;", filtroCol);
    }
    else
    {
        snprintf(sql, sizeof(sql),
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        escrito = snprintf(objeto, sizeof(objeto),
            ...
            "\"dosagem\":%s,\"frequencia\":%s,\"observacoes\":%s}",
            primeiro ? "" : ",",
            id, pacienteId, medicoId, medicamentoJson, dosagemJson,
            frequenciaJson, observacoesJson);
